...
@section('child-content')
    <div class="relative h-screen log-bg flex flex-col items-center pb-28 login-bg overflow-auto font-Figtree">
        ...
        <a href="{{ route('frontend.index') }}">
            <img class="mt-11 dark:hidden w-[175px] h-[42px] object-contain" src="{{ $logoLight }}"
                alt="{{ __('Logo') }}">
            <img class="mt-11 hidden dark:block w-[157px] h-[42px] object-contain" src="{{ $logoDark }}"
                alt="{{ __('Logo') }}">
        </a>
        <div
            class="relative bg-white dark:bg-[#3A3A39] rounded-3xl w-[350px] xs:w-[388px] sm:w-[506px] h-max px-4 sm:px-10 py-8 z-[2] mt-11">
            <p class="text-center text-24 font-bold text-color-14 dark:text-white">
                {{ __('Sign in to :x', ['x' => preference('company_name')]) }}</p>
            @if (is_array($preference) && count($preference) > 0)
                @if (in_array('Google', $preference))
                    <a class="relative block text-center bg-[#4285F4] rounded-xl py-3 mt-6 text-white text-16 font-semibold"
                        href="{{ route('login.google') }}">{{ __('Continue with Google') }}
                        <div class="absolute left-1 top-1 social-icon-rtl">
                            <svg class="neg-transition-scale" width="40" height="40" viewBox="0 0 40 40"
                                fill="none" xmlns="http://www.w3.org/2000/svg">
                                <rect width="40" height="40" rx="8" fill="white" />
                                <path
                                    d="M31.0001 20.2444C31.0001 19.34 30.9252 18.68 30.7632 17.9956H20.2246V22.0778H26.4105C26.2858 23.0922 25.6124 24.62 24.1157 25.6466L24.0948 25.7833L27.4269 28.313L27.6577 28.3356C29.7779 26.4167 31.0001 23.5933 31.0001 20.2444Z"
                                    fill="#4285F4" />
                                <path
                                    d="M20.2245 31C23.2551 31 25.7993 30.0222 27.6576 28.3355L24.1156 25.6466C23.1678 26.2944 21.8957 26.7466 20.2245 26.7466C17.2563 26.7466 14.737 24.8278 13.839 22.1755L13.7073 22.1865L10.2426 24.8143L10.1973 24.9377C12.0431 28.531 15.8344 31 20.2245 31Z"
                                    fill="#34A853" />
                                <path
                                    d="M13.839 22.1756C13.602 21.4912 13.4649 20.7578 13.4649 20C13.4649 19.2422 13.602 18.5089 13.8265 17.8245L13.8202 17.6787L10.312 15.0087L10.1973 15.0622C9.43651 16.5533 9 18.2278 9 20C9 21.7723 9.43651 23.4467 10.1973 24.9378L13.839 22.1756Z"
                                    fill="#FBBC05" />
                                <path
                                    d="M20.2245 13.2533C22.3322 13.2533 23.7539 14.1455 24.5646 14.8911L27.7324 11.86C25.7869 10.0878 23.2551 9 20.2245 9C15.8344 9 12.0431 11.4689 10.1973 15.0622L13.8265 17.8245C14.737 15.1722 17.2563 13.2533 20.2245 13.2533Z"
                                    fill="#EB4335" />
                            </svg>
                        </div>
                    </a>
                ...
                @if (in_array('Facebook', $preference))
                    <a class="relative block text-center bg-[#4285F4] rounded-xl py-3 mt-6 text-white text-16 font-semibold"
                        href="{{ route('login.facebook') }}">{{ __('Continue with Facebook') }}
                        <div class="absolute left-1 top-1 social-icon-rtl">
                            <svg class="neg-transition-scale" width="40" height="40" viewBox="0 0 40 40"
                                fill="none" xmlns="http://www.w3.org/2000/svg">
                                <rect width="40" height="40" rx="8" fill="white" />
                ...
                <div class="flex items-center justify-center mt-5">
                    <svg class="w-10 xs:w-[60px] sm:w-[120px] text-color-DF dark:text-color-47" height="1"
                        viewBox="0 0 122 1" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <line y1="0.5" x2="122" y2="0.5" stroke="currentColor" />
                    </svg>
                    <p class="text-color-14 dark:text-white text-14 px-2 text-center">
                        {{ __('Or sign in with your email') }}</p>
                    <svg class="w-10 xs:w-[60px] sm:w-[120px] text-color-DF dark:text-color-47" height="1"
                        viewBox="0 0 122 1" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <line y1="0.5" x2="122" y2="0.5" stroke="currentColor" />
                    </svg>
            ...
            @if (session('resendCode'))
                <p class="text-13 xs:text-15 font-normal text-color-14 dark:text-white mt-6 text-center">
                    {!! str_replace(
                        __('Click here to verify your email.'),
                        '<a class="text-15 font-semibold underline" href=' .
                            route('users.resend.code', ['email' => session('email')]) .
                            '>' .
                            __('Click here to verify your email.') .
                            '</a>',
                        __('Click here to verify your email.'),
            ...
            <form action="{{ route('login.post') }}" method="post" class="loginForm button-need-disable">
                ...
                <label class="block mt-5">
                    ...
                    <input id="login-email" type="email"
                        class="form-control border border-color-89 dark:border-[#474746] rounded-xl h-12 w-full mt-1.5 px-2 text-color-14 dark:text-white dark:bg-[#333332] text-14 font-medium"
                        value="{{ old('email') }}" name="email" placeholder="{{ __('Email Address') }}" required
                ...
                <div class="block mt-6">
                    <span class="flex justify-between">
                        ...
                        <a href="{{ route('login.reset') }}"
                            class="font-semibold underline block text-14 font-medium text-color-14 dark:text-white">{{ __('Forgot Password?') }}</a>
                    </span>
                    <div class="relative password-container">
                        <input id="login-password" type="password"
                            class="form-control border border-color-89 dark:border-[#474746] rounded-xl h-12 w-full mt-1.5 px-2 text-color-14 dark:text-white dark:bg-[#333332] text-14 font-medium password-field"
                            name="password" placeholder="{{ __('Password') }}" required
                            oninvalid="this.setCustomValidity('{{ $msg }}')" />
                        <a href="javascript: void(0)">
                            <svg class="absolute right-[14px] top-5 pass-eye" width="20" height="20"
                            ...
                            <svg class="password-show absolute right-[14px] top-5 pass-eye" width="20" height="20"
                                viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                                <g>
                                    <path
                                        d="M9.99967 3.75C5.83301 3.75 2.27467 6.34167 0.833008 10C2.27467 13.6583 5.83301 16.25 9.99967 16.25C14.1663 16.25 17.7247 13.6583 19.1663 10C17.7247 6.34167 14.1663 3.75 9.99967 3.75ZM9.99967 14.1667C7.69967 14.1667 5.83301 12.3 5.83301 10C5.83301 7.7 7.69967 5.83333 9.99967 5.83333C12.2997 5.83333 14.1663 7.7 14.1663 10C14.1663 12.3 12.2997 14.1667 9.99967 14.1667ZM9.99967 7.5C8.61634 7.5 7.49967 8.61667 7.49967 10C7.49967 11.3833 8.61634 12.5 9.99967 12.5C11.383 12.5 12.4997 11.3833 12.4997 10C12.4997 8.61667 11.383 7.5 9.99967 7.5Z"
                            ...
                            <svg class="password-hide absolute right-[14px] top-5 pass-eye" width="20" height="20"
                                viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                                <path
                ...
                <button
                    class="block w-full bg-color-14 dark:bg-white text-white dark:text-color-14 text-16 font-semibold py-3 flex justify-center items-center gap-3 rounded-xl text-center mt-6">{{ __('Sign in') }}
                ...
                @if (config('openAI.is_demo') || str_contains(url()->current(), 'localhost'))
                    <div class="text-lg flex items-center mt-5">
                        <hr class="border border-gray-2 w-full">

                        <p
                            class="roboto-regular text-gray-10 text-center text-sm md:text-base dark:text-white px-3 md:px-5 leading-5 whitespace-nowrap">
            ...
            @if (preference('customer_signup') == '1')
                <div>
                    <p class="text-13 xs:text-15 font-normal text-color-14 dark:text-white mt-6 text-center">
                        {{ __('Don’t have an account?') }} <a class="text-15 font-semibold underline"
                            href="{{ route('users.registration') }}">{{ __('Register for free') }}</a> </p>
                </div>
            @endif
            <img class="absolute bottom-[132px] left-[-117px] z-[-11] hidden md:block robot-log neg-transition-scale"
                src="{{ asset('Modules/OpenAI/Resources/assets/image/robotlog.png') }}" alt="robotlog">
        </div>
    </div>
@endsection
