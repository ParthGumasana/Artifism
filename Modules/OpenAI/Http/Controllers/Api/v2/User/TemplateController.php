    ...
    public function generate(TemplateRequest $request): JsonResponse
    {
        ...
        if (! subscription('isAdminSubscribed')) {
            ...
            if ($userStatus['status'] == 'fail') {
                return response()->json(['error' => $userStatus['message']], Response::HTTP_UNPROCESSABLE_ENTITY);
            }
            ...
            if ($validation['status'] == 'fail' && ! auth()->user()->hasCredit('word')) {
                return response()->json(['error' => $validation['message']], Response::HTTP_UNPROCESSABLE_ENTITY);
            }
        }
