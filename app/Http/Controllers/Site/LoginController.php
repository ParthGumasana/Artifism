    ...
    public function registration(RegistrationUserRequest $request, SubscriptionService $subscriptionService, FolderService $folderService)
    {
        if (!$request->isMethod('post')) {
            ...
            if ($encryptedKey) {
                ...
                if (isset($separator[1]) && User::where('email', $separator[1])->exists()) {
                    return abort(401);
                }
            }
            if (isActive('Affiliate')) {
            ...
            if (strpos($request->password, "\\0") !== false) {
                return redirect()->back()->withInput()->withErrors(['fail' => __("Password cannot contain null characters.")]);
            }
            ...
            try {
                ...
                if (!empty($id)) {
                    ...
                    $teamId = (new TeamMemberService())->teamMemberStore($request, $id);
                ...
                DB::rollBack();
                return redirect()->back()->withInput()->withErrors(['fail' => $e->getMessage()]);
            }
